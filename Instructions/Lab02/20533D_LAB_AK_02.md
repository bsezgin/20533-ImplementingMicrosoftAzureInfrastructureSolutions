...
2. To create a new resource group, type the following command, and then press Enter (replace **`AzureRegion`** with the name of the same Azure region you chose in the previous exercise):

  ```
  $rg = New-AzureRMResourceGroup -Name '20533D0204-LabRG' -Location 'AzureRegion'
  ```

3. To create a new virtual network named **20533D0204-vnet** with the address space **10.11.0.0/16** and store a reference to it in the **`$vnet`** variable, type the following command, and then press Enter:
...
4. To add a subnet to the new virtual network, type the following command, and then press Enter:

  ```
  Add-AzureRmVirtualNetworkSubnetConfig -Name 'Subnet1' -VirtualNetwork $vnet -AddressPrefix '10.11.0.0/24'
  ```

5. To update the configuration in the virtual network, type the following command, and then press Enter:
...
3. To create a new virtual network named **20533D0205-vnet** with the address space **10.12.0.0/16** and a subnet named Subnet1 with the address prefix of 10.12.0.0/24, type the following command, and then press Enter (replace **`AzureRegion`** with the name of the same Azure region you chose in the previous exercise):
